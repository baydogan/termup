...
// pollInterval is how often the dashboard re-fetches from ztd. Kept below the
// probe interval so the view stays fresh (probe cadence is the server's).
const pollInterval = 10 * time.Second

// resolveAddr picks the server address: --addr flag > ZT_ADDR env > default.
...
func main() {
	...
	if len(args) == 0 || args[0] != "watch" {
		fmt.Fprintln(os.Stderr, "usage: zt [--addr ...] watch")
		os.Exit(2)
	}
...
func (m model) View() string {
	header := titleStyle.Render("zerotolerance") + "  " +
	...
	switch {
	case m.err != nil:
		body = errStyle.Render("cannot reach ztd: "+m.err.Error()) +
